	   ...
	   )
{
	...
	do
	{
		...
		for (i = 0; i < NUM_SYM; i++)
		{
			...
			if ( (sym = sym_callback(s_type, s_name, s_subname)) == 0 ) {
				break;
			}
			...
			if (RegSetValueEx(h_key, name, 0, REG_DWORD, pv(&sym), sizeof(sym)) != 0) {
				break;
			}
		}
		...
		/* save access key */
		if (RegSetValueEx(h_key, "sc_key", 0, REG_DWORD, pv(&acc_key), sizeof(u32)) != 0) {
			break;
		}
		succs = 1;
	} while (0);
